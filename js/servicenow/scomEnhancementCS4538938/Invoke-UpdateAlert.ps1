...
param(
    ...
    if($scomServerUsername -ne $null -And $useWindowsAuth -ne "true" ){
...
"JSON $json"
   ...
   foreach($alertObj in $JsonObj){
    ...
    # CS4538938 - Use moids to reset monitors for alerts we just closed
    # Convert string to array
    $moidsObj = $moids | ConvertFrom-Json
    foreach($scom in $moidsObj){
        $alert = Get-SCOMAlert -Id $scom.alertId
        $monitor = Get-SCOMMonitor -Id $alert.MonitoringRuleId
        Get-SCOMClassInstance -Id $scom.moid | $_.ResetMonitoringState($monitor)
    }
    exit 0;
}